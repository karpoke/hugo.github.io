---
title: "Sobre mí"
layout: "page"
url: "/sobre-mi/"
summary: "Información sobre Ignacio Cano"
---

## Hola, soy Ignacio Cano 👋

Bienvenido a mi blog técnico.

## ¿Qué encontrarás aquí?

En este blog escribo sobre:

- **Desarrollo web**: Frameworks modernos, mejores prácticas y arquitecturas
- **Python**: Desde fundamentos hasta temas avanzados
- **DevOps**: CI/CD, contenedores, automatización
- **Bases de datos**: Optimización, modelado, NoSQL vs SQL
- **Experiencias técnicas**: Soluciones a problemas reales

## Contacto

Puedes encontrarme en:

- **GitHub**: [@karpoke](https://github.com/karpoke/)
- **Email**: Disponible próximamente

---

*Última actualización: Febrero 2026*
